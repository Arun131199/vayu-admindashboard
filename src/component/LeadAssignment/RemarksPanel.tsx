...
import { addLeadRemark, getLeadRemarks, type LeadRemark, type LeadType } from "../../service/leadApi";
...
export default function RemarksPanel({ leadType, entityId }: RemarksPanelProps) {
    ...
    const fetchRemarks = async () => {
        setLoading(true);
        try {
            const data = await getLeadRemarks(leadType, entityId);
            setRemarks(data);
        } catch (err) {
    ...
    const handleAddRemark = async () => {
        ...
        setError("");
        try {
            await addLeadRemark(leadType, entityId, newRemark.trim());
            setNewRemark("");
            await fetchRemarks();
        } catch (err: any) {
